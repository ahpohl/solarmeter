...
bool Solarmeter::Setup(const std::string &config) {
  ...
  if (!Inverter->ReadCumulatedEnergy(Datagram.TotalEnergy,
                                     CumulatedEnergyEnum::LIFETIME_TOTAL)) {
    ErrorMessage = Inverter->GetErrorMessage();
    return false;
  }
  std::cout << "Current meter reading: " << Datagram.TotalEnergy << " kWh"
            << std::endl;
...
bool Solarmeter::Receive(void) {
  ...
  Datagram.MfgDate =
      std::string("Year ") + mfg_date.Year + " Week " + mfg_date.Week;
  ...
  if (!Inverter->ReadDspValue(Datagram.GridVoltage,
                              DspValueEnum::GRID_VOLTAGE)) {
    ErrorMessage = Inverter->GetErrorMessage();
    return false;
  }
  if (!Inverter->ReadDspValue(Datagram.GridCurrent,
                              DspValueEnum::GRID_CURRENT)) {
    ErrorMessage = Inverter->GetErrorMessage();
    return false;
  }
  ...
  if (!Inverter->ReadDspValue(Datagram.InverterTemp,
                              DspValueEnum::TEMPERATURE_INVERTER)) {
    ErrorMessage = Inverter->GetErrorMessage();
    return false;
  }
  if (!Inverter->ReadDspValue(Datagram.BoosterTemp,
                              DspValueEnum::TEMPERATURE_BOOSTER)) {
    ErrorMessage = Inverter->GetErrorMessage();
    return false;
  }
  if (!Inverter->ReadDspValue(Datagram.RIso,
                              DspValueEnum::ISOLATION_RESISTANCE)) {
    ErrorMessage = Inverter->GetErrorMessage();
    return false;
  }
  try {
...
bool Solarmeter::Publish(void) {
  ...
  if (Mqtt->GetNotifyOnlineFlag() ||
    ...
    if (!(Mqtt->PublishMessage(
            oss.str(), Cfg->GetValue("mqtt_topic") + "/state", 0, false))) {
      ErrorMessage = Mqtt->GetErrorMessage();
      return false;
    }
