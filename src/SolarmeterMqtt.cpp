...
bool SolarmeterMqtt::SetUserPassAuth(const std::string &user,
...
bool SolarmeterMqtt::SetTlsConnection(const std::string &cafile,
...
bool SolarmeterMqtt::Connect(const std::string &host, const int &port,
                             const int &keepalive) {
  int rc = 0;
...
bool SolarmeterMqtt::SetLastWillTestament(const std::string &message,
...
bool SolarmeterMqtt::PublishMessage(const std::string &message,
...
std::string SolarmeterMqtt::GetErrorMessage(void) const { return ErrorMessage; }

bool SolarmeterMqtt::GetConnectStatus(void) const { return IsConnected; }
...
void SolarmeterMqtt::OnConnectCallback(struct mosquitto *mosq, void *obj,
                                       int connack_code) {
  if (!connack_code) {
    IsConnected = true;
...
void SolarmeterMqtt::OnConnectCallbackWrapper(struct mosquitto *mosq, void *obj,
                                              int connack_code) {
  auto *p = reinterpret_cast<SolarmeterMqtt *>(obj);
  return p->SolarmeterMqtt::OnConnectCallback(mosq, obj, connack_code);
}

void SolarmeterMqtt::LogCallback(struct mosquitto *mosq, void *obj, int level,
                                 const char *str) {
...
void SolarmeterMqtt::LogCallbackWrapper(struct mosquitto *mosq, void *obj,
